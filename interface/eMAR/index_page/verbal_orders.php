<div class="row" id="verbal_orders" style="display: none;">
    ...
    <div class="col-md-12">
            ...
            <div class="col-6 col-md-6">
                <div class="row">
                    <?php if (array_key_exists($_SESSION['authUserID'], $GLOBALS['approved_by'])) { ?>
                        <div class="col-12 col-md-12" id="sign_all" style="display: none;">
                            <a href="javascript:void(0);" class="btn-default" id="sign_all_medo">Approve and Sign</a>
                        </div>
                        <!-- <div class="col-6 col-md-6" id="signature_btn" style="display: none;text-align: end;">
                            <a href="javascript:void(0);" class="btn-default" id="staff_sign">Approve and Sign</a>
                        </div>
                        <div class="col-6 col-md-6" id="signature_btn_do" style="display: none;text-align: end;">
        ...
        <div class="row m-3">
            <div class="col-12 col-md-12">
                <b>
                    <h6><input type="checkbox" id="my_sign_all"> Check All Orders </h6>
                </b>
            </div>
        </div>
        <table id="" class="table e_MAR_tb-1" style="width:100%">
            ...
            <tbody>
                ...
                $sql_query = "select prescriptions.id,prescriptions.is_prn,prescriptions.provider_id,prescriptions.active,prescriptions.order_set,prescriptions.verbal ,patient_data.fname, patient_data.lname, `date_added`, `drug`,`quantity`, `med_brought_in_id`, prescriptions.`date_modified`,prescriptions.`updated_by`,prescriptions.`cancelled_provider`,prescriptions.`cancelled_reason`,prescriptions.`cancelled_verbal`,
                        ...
                        from prescriptions, patient_data where patient_data.pid=prescriptions.patient_id and `prescriptions`.p_delete='" . $p_delete . "' and prescriptions.`provider_id` = '" . $_SESSION['authUserID'] . "' and patient_data.pid='" . $pid . "' ORDER BY is_prn ASC, start_date ASC";
                try {
                    $res = sqlStatement($sql_query);

                    while ($developer = sqlFetchArray($res)) {
                        $qry_sign = "SELECT * FROM `medlogs_signature` JOIN `medlogs_signature_header` ON `medlogs_signature`.`header_id`=`medlogs_signature_header`.`header_id` WHERE `medlogs_signature`.`prescription_id`='" . $developer['id'] . "' AND active='" . $developer['active'] . "'";
                            ...
                            <tr id="<?php echo $developer['id']; ?>" class="<?= $class ?>">
                                <td class="<?= $class2 ?>">
                                    <?php if (!$detail_sign) {  ?>
                                        ...
                                        <?php
                                        $qry_sign_ms = "SELECT * FROM `medlogs_signature` JOIN `medlogs_signature_header` ON `medlogs_signature`.`header_id`=`medlogs_signature_header`.`header_id` WHERE `medlogs_signature`.`prescription_id`='" . $developer['id'] . "' ";
                                        $result_sign_ms = sqlStatement($qry_sign_ms);
                                        $detail_sign_ms = sqlFetchArray($result_sign_ms);
                                        if ($detail_sign_ms) { ?>
                                            <span style="color: green;font-weight: 900;">For Active Medication Signed By <?= $providers[$detail_sign_ms['staff_id']]['fname']; ?> <?= $providers[$detail_sign_ms['staff_id']]['lname']; ?></span>
                                            <p><?= date('m-d-Y H:i:s', strtotime($detail_sign_ms['signed_at'])) ?> </p>

                                        <?php }
                                    } else { ?>

                                        <?php
                                        $qry_sign_ms = "SELECT * FROM `medlogs_signature` JOIN `medlogs_signature_header` ON `medlogs_signature`.`header_id`=`medlogs_signature_header`.`header_id` WHERE `medlogs_signature`.`prescription_id`='" . $developer['id'] . "' AND active !='" . $developer['active'] . "'  ";
                                        $result_sign_ms = sqlStatement($qry_sign_ms);
                                        $detail_sign_ms = sqlFetchArray($result_sign_ms);
                                        if ($detail_sign_ms) { ?>
                                            <span style="color: green;font-weight: 900;">For Active Medication Signed By <?= $providers[$detail_sign_ms['staff_id']]['fname']; ?> <?= $providers[$detail_sign_ms['staff_id']]['lname']; ?></span>
    ...
    <div class="col-md-12">
        <div class="row">
            ...
            <div class="col-6 col-md-6" id="signature_btn_do1" style="display: none;">
                <!-- <div class="row">
                    <?php if (array_key_exists($_SESSION['authUserID'], $GLOBALS['approved_by'])) { ?>
                        <div class="col-12 ">
                            <a href="javascript:void(0);" class="btn-default" id="staff_sign_do">Approve and Sign</a>
        ...
        <table id="" class="table e_MAR_tb">
            ...
            <tbody>
                <?php
                $sql_query = "SELECT * FROM `doctors_order` where pid = '" . $pid . "' AND `provider_id` = '" . $_SESSION['authUserID'] . "' AND p_delete='" . $p_delete . "' ";
                $res = sqlStatement($sql_query);
                while ($doctor_result = sqlFetchArray($res)) {

                    $qry_sign = "SELECT * FROM doctors_order_signature WHERE doctors_orders_id='" . $doctor_result['id'] . "' AND active='" . $doctor_result['discontinued'] . "' ";
                    $result_sign = sqlStatement($qry_sign);
                    $detail_sign = sqlFetchArray($result_sign);
                    ...
                    <tr class="<?= $class ?>" id="<?php echo $doctor_result['id']; ?>" <?php if ($doctor_result['discontinued'] == 1) { ?> style="background-color: #D5DBDB;filter: blur(1px);" <?php } ?>>
                        <td class="<?= $class2 ?>" style="text-align:center;padding-left: 30px;">
                            <?php if (!$detail_sign) { ?>
                                ...
                                <?php
                                $qry_sign_doo = "SELECT * FROM doctors_order_signature WHERE doctors_orders_id='" . $doctor_result['id'] . "' AND active !='" . $doctor_result['discontinued'] . "' ";
                                $result_sign_doo = sqlStatement($qry_sign_doo);
                                $detail_sign_doo = sqlFetchArray($result_sign_doo);
                                if ($detail_sign_doo) { ?>
                                    <span style="color: green;font-weight: 900;padding-left: 10px;">For Active Doctors Orders Signed By <?php echo $providers[$detail_sign['signer_id']]['fname']; ?> <?php echo $providers[$detail_sign['signer_id']]['lname']; ?></span>
                                    <p><?= date('m-d-Y H:i:s', strtotime($detail_sign['signed_at'])) ?></p>
                                <?php } ?>
    ...
    <div class="col-md-12">
        ...
        <div class="row py-2">
            ...
            <div class="col-6 col-md-6">
                <h4 class="text-bold" style="font-weight: 900;">Other Orders</h4>
            </div>
