...
$sql_qry = "SELECT * FROM orderset_medication WHERE orderset_id=". $orderset_id ." ORDER BY day ASC";
$med_res = sqlStatement($sql_qry);

$pdf->WriteHTML($style_sheet, \Mpdf\HTMLParserMode::HEADER_CSS);

$pdf->writeHTML(print_orderset($orderset, $med_res), 2);
$pdf->SetTitle($orderset['name']);
echo $pdf->Output('form.pdf', 'I');

function print_orderset($orderset, $med_res){
    $html =  '<html>
