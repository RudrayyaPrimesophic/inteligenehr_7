...
if (sqlStatement($sql_qry)) {
    ...
    foreach ($prescriptions as $key => $value) {
        ...
        if (sqlStatement($sql)) {
            log_user_event('eMAR-Verbal Orders', 'Verbal Order Signed For Patient -' . $pid, $_SESSION['authUserID']);
            $data = ['status' => true, 'id' => $msh['header_id']];
        } else {
            log_user_event('eMAR-Verbal Orders', 'Failed To Sign Verbal Order For Patient -' . $pid, $_SESSION['authUserID']);
            $data = ['status' => false];
        }
    }
} else {
